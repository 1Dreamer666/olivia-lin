...
BASE = Path(__file__).resolve().parent.parent  # BSide_Olivia_Lin/


def _read(rel: str) -> str:
    p = BASE / rel
    if not p.exists():
        raise FileNotFoundError(f"skill 文件缺失: {p}")
    return p.read_text(encoding="utf-8").strip()


def build_system_prompt(include_samples: bool = True, time_hint: str | None = None) -> str:
    parts = [
